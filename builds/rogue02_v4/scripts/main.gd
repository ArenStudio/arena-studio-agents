...
func _run_batch_eval(start_seed: int, end_seed: int) -> void:
	...
	print("=== ПОЛНЫЙ ЗАМЕР ПО 85 ЗАРАНЕЕ ОБЪЯВЛЕННЫМ СИДАМ (%d–%d, N=%d) ===" % [start_seed, end_seed, N])
	print("1. ВИНРЕЙТ (AC1 / AC2):")
	...
	print("\n3. ПРАВИЛО BAL-001 (Доли ресурсных действий внутри класса, 10–60%%):")
...
func _ai_decide_and_act() -> void:
	...
	if player.can_afford(GameState.COST_DASH):
		...
		# C. Gap-close on Ranged Archer at distance 3
		for d in dirs:
	...
	if not adjacent_enemies.is_empty():
		...
		# C. Block incoming attack (>= 3 dmg) if not stunned
		var incoming_attack: bool = false
		for e in adjacent_enemies:
			if e.intent_type == Enemy.IntentType.ATTACK and e.intent_damage >= 3 and not e.is_stunned:
	...
	# 5. Collect nearby Altars of Focus or Chests on floor (within distance 8)
	var target_pickup: Vector2i = Vector2i(-1, -1)
	var min_pickup_dist: int = 9
	...
	# 6. Navigate directly to Exit stairs
