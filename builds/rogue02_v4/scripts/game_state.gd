...
# ==============================================================================
# SYS PARAMETERS & CONSTANTS (PROJECT: ROGUE-02 / SYS-004 & SYS-005)
# ==============================================================================
...
# Guaranteed package by SYS-005
const ENEMIES_PER_FLOOR: Array[int] = [3, 3, 4, 4, 5, 5, 5, 6]
const ALTAR_FLOORS: Array[int] = [2, 4, 6, 8] # Guaranteed 4 altars across full run
const CHEST_FLOORS: Array[int] = [3, 6]       # Guaranteed 2 caches across full run
const ALTAR_FOCUS_REWARD: int = 8
const CHEST_HP_REWARD: int = 4
const CHEST_FOCUS_REWARD: int = 8
const ALTAR_HP_REWARD: int = 4
const STAIRS_HEAL: int = 2                    # +2 HP per transition (7 transitions)
...
# Mechanic counters (AC7 & D-002)
var counters = {
	"focus_gained": 0,
	"focus_gained_kills": 0,
	"focus_gained_altars": 0,
	"focus_gained_chests": 0,
	"focus_spent_dash": 0,
	"focus_spent_block": 0,
	"focus_spent_crush": 0,
	"dash_used": 0,
...
# Full State Hash Accumulator (D-003: FNV-1a 32-bit hash)
var state_hash: int = 2166136261
...
func set_seed(seed_val: int) -> void:
	...
	state_hash = (2166136261 ^ (seed_val & 0xFFFFFFFF)) & 0xFFFFFFFF

func hash_u32(val: int) -> void:
	state_hash = (state_hash ^ (val & 0xFFFFFFFF)) & 0xFFFFFFFF
	state_hash = (state_hash * 16777619) & 0xFFFFFFFF
